  ...
  <div class="">
    <div class="g_row_and_center_between recommend">
      <div class="left" v-if="recommendList.length > 0">
        <div class="title" style="margin: 0.2rem 0.2rem 0.2rem 0">{{ checkedRecommend.title }}</div>
        <div class="detail" style="margin: 0.2rem 0.2rem 0.2rem 0">{{ checkedRecommend.detail }}</div>
        <div class="" style="margin: 0.2rem 0.2rem 0.2rem 0">
          <el-button icon="el-icon-caret-right" class="playButton"
            @click="goVideoDetail(checkedRecommend.id,checkedRecommend.setid)">播放正片</el-button>
        </div>
      </div>
      ...
      <div class="right g_row_and_center_between">
        ...
        <div v-if="recommendListPage.length > 0" @click="goVideoDetail(item.id,item.setid)" style="  cursor: pointer;"
          v-for="(item, index) in recommendListPage" :key="index" class="g_vertical_center">
