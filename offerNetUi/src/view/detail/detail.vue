...
<!-- 不懂    -->
      ...
      <div class="play">
        <div class="playleft">
          <div class="videoDiv">
          <vue-core-video-player  :src="videoSource.src" :cover="videoSource.cover"  autoplay loop="true"/>
          </div>
...
<!--            视频信息-->
            ...
            <div class="episodeTitle">
              {{ videoSet.title }} {{ checkVideo.name }}
            </div>
            <div class="tagDiv">
...
<!--        视频右侧导航栏-->
        <div class="playright">
          <!--        上半部分-->
          <div class="padding20">
            <div class="videoTitle">{{ videoSet.title }}</div>
...
<!--            下半部分-->
            <div class="episode">
              <div class="episodeType" @click=" changeEpisodeType('1')"
                :style="checkdEpisodeType == '1' ? 'color: #ff6022;' : ''">
                剧集与更新
              ...
              <div class="episodeType" @click=" changeEpisodeType('2')"
                ...
                为您推荐
                <div v-if="checkdEpisodeType == '2'" class="sold"></div>
              </div>
            </div>
            <!--            剧集按钮-->
          ...
          <div class="footLeft">
            ...
            <div class="commentDiv">
              ...
              <div class="commentParent"
                v-for="(                                    comment, index                                    ) in                                     commentParentList                                    "
                :key="index">
                ...
                <div class="commentAction">
                  ...
                  <span class="iconfont iconfont-huifu" @click=" off_on_commentBox(comment, [index])"
                    style="font-size: 0.25rem;font-weight: bolder;  cursor: pointer;"><span
                      style="font-size: 0.15rem;">{{ comment.replyCount }}</span></span>
                </div>
  ...
  methods: {
    ...
    initVideoSet() {
      ...
      api.getVideoSetById(param).then((res) => {
        if (res.code == 200) {
          this.videoSet = res.data;
          this.videoSource.src = res.data.videoSrc;
          this.videoSource.resolution =res.data.resolution;
          this.videoSource.cover =res.data.cover;
    ...
    comment(param) {
      api.addComment(param).then((res) => {
        if (res.code == 200) {
          ...
          this.commentParentList.unshift(res.data);
          this.commentTotal++;
        } else {
    ...
    parentComment(content) {
      let param = {
        sourceId: this.routeVideoId,//视频id
